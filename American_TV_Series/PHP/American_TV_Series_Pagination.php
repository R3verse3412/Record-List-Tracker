...
if (!empty($search)) {
    ...
    $search_query = "WHERE name LIKE '%" . $conn->real_escape_string($search) . "%' 
                     ...
                     OR director LIKE '%" . $conn->real_escape_string($search) . "%'";
}

// Fetch total number of records based on search
$count_query = "SELECT COUNT(*) AS total FROM american_tv_series $search_query";
$count_result = $conn->query($count_query);
$total_records = $count_result->fetch_assoc()['total'];

// Fetch movies for the current page based on search
$query = "SELECT * FROM american_tv_series $search_query LIMIT $limit OFFSET $offset";
