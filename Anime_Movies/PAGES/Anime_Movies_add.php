...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['img']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($img_tmp_name, $img_path)) {
            ...
            $sql = "INSERT INTO anime_movies (user_id, name, summary, year, genre, rating, img, duration, studio, date_added) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $stmt = $conn->prepare($sql);

            // Bind parameters
            $stmt->bind_param("ississsis",$user_id, $name, $summary, $year, $genre, $rating, $img_path, $duration, $studio);

            if ($stmt->execute()) {
                $_SESSION['success_message'] = "Movies added successfully!";
...
?>
    ...
    <div class="container">
        <div class="text-center mb-4">
            <h3>Add New Anime Movies</h3>
            <p class="text-muted">Complete the form below to add a new Anime Movies</p>
        </div>
